...
std::any ASTGenerator::visitArray_type(GrammarParser::Array_typeContext* ctx) {
  return new Instruction(
      "ArrayType",
      {std::any_cast<Instruction*>(visit(ctx->base_type())),
       new Instruction("NumberOfElements", {std::any_cast<Instruction*>(
...
std::any ASTGenerator::visitClass_statement(
  ...
  if (ctx->method()) {
    return new Instruction("Class statement",
...
std::any ASTGenerator::visitMethod(GrammarParser::MethodContext* ctx) {
  std::vector<Instruction*> children;
  children.push_back(new Instruction(
      "Name", {new Instruction(ctx->VARIABLE_NAME()->toString(), {})}));
  children.push_back(std::any_cast<Instruction*>(visit(ctx->type())));
  if (ctx->parameter_list()) {
    children.push_back(
        std::any_cast<Instruction*>(visit(ctx->parameter_list())));
  }
  for (auto* stat : ctx->statement()) {
    children.push_back(std::any_cast<Instruction*>(visit(stat)));
  }
...
std::any ASTGenerator::visitParameter_list(
    GrammarParser::Parameter_listContext* ctx) {
  std::vector<Instruction*> children;
...
std::any ASTGenerator::visitType(GrammarParser::TypeContext* ctx) {
  if (ctx->base_type()) {
    return new Instruction(
        "Type", {std::any_cast<Instruction*>(visit(ctx->base_type()))});
  }
  return new Instruction(
      "Type", {std::any_cast<Instruction*>(visit(ctx->array_type()))});
}
...
std::any ASTGenerator::visitPre_main(GrammarParser::Pre_mainContext* ctx) {
  if (ctx->function_declaration()) {
    return new Instruction(
        "Pre_main",
        {std::any_cast<Instruction*>(visit(ctx->function_declaration()))});
  }
  return new Instruction("Pre_main",
                         {std::any_cast<Instruction*>(visit(ctx->class_()))});
}

std::any ASTGenerator::visitFunction_declaration(
    GrammarParser::Function_declarationContext* ctx) {
  std::vector<Instruction*> children;
  children.push_back(std::any_cast<Instruction*>(visit(ctx->type())));
